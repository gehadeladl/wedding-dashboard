...
export default function TiesBowtiesPage() {
  const [products, setProducts] = useState([]);
  const [loading, setLoading] = useState(true);

  const getProducts = async () => {
    try {
      const res = await api.get("/website/products?section=TIES_BOWTIES");
    ...
    <WebsiteLayout>
      <section
        style={{
          ...
          background:
            "linear-gradient(135deg, #3b0764 0%, #581c87 45%, #111827 100%)",
          color: "#fff",
          textAlign: "center",
        }}
      >
        ...
        >
          ...
          >
            لمسات أنيقة تكمل الإطلالة الرسمية بتصميمات متنوعة وخامات مميزة.
          </p>
        </motion.div>
      </section>
